    ...
    def run(self):
        ...
        try:
            ...
            if len(argument) == 1:
                ...
                if len(factors) != 0:
                    Fit_Binomial_Model(self.path_for_csv, self.path_for_result, self.name_of_file, argument, factors)
                else:
...
def Fit_Binomial_Model(path_to_csv, output_path, output_file, argument, factors):

    name_for_Cook = os.path.splitext(output_file)[0] + "_Cook_dst.png"
    ...
    if not os.path.exists(output_path):
        ...
        if path.isfile(dst_path):
            ...
            new_name = str(int(cnt_of_files_in_dir/2) + 1) + '_' + output_file
            dst_path = path.join(output_path, new_name)

            new_name_Cook = str(int(cnt_of_files_in_dir/2) + 1) + '_' + name_for_Cook
