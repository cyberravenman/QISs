        ...
        try:
            if self.flag_radio_button:
                ...
                self.full_path_for_NIST = Calc_QRNG(self.token_name, self.ibm_channel, self.backend_name,
                                                    ...
                                                    self.num_of_qubits, 0, self.timezone)
            else:
                ...
                if len(checked_qubits) != 0:
                    self.full_path_for_NIST = Calc_QRNG(self.token_name, self.ibm_channel, self.backend_name,
                                                        int(self.shots), float(self.alpha), int(self.opt_level),
                                                        self.num_of_qubits_max, checked_qubits, self.timezone)
                else:
...
def Calc_QRNG(user_token, ibm_channel, backend_name,
              ...
              num_of_qubits, usr_qubits, time_delta):
    print(f"\n-----КГСЧ: '{backend_name}'-----\n")
    ...
    with Session(backend=backend) as session:
        ...
        csv_str_1 = "Project_Type,Status,Date,Time,JobID,Backend,Shots,Alpha,Quality"
        ...
        for i in range(0, USER_QUBITS_AND_CLBITS):
            ...
            if float(p_value) >= ALPHA:
                ...
                file_good_q.close();    file_cnt_q_good.close();    csv_file_cnt_q_good.close();    csv_file_cnt_q_all.close()
            else:
                ...
                file_bad_q.close(); file_cnt_q_bad.close(); csv_file_cnt_q_bad.close(); csv_file_cnt_q_all.close()
