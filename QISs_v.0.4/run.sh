...
if [ -d "$PWD/QISs_v.0.4-venv" ]; then
	echo '[*]: Activate Python virtual env..'
	source $PWD/QISs_v.0.4-venv/bin/activate &>-
	echo '[+]: Done'
else
	echo '[*]: Updating..'
	sudo apt update
	...
	python3 -m venv QISs_v.0.4-venv
	echo '[+]: Done'
...
python3 QIS_Benchmark_v.0.4.py
