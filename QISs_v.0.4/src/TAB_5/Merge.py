...
class MergeCsv(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            print("Возникло непредвиденное исключение. Перепроверьте исходные данные.\n", ex)
        print("\n-----ВЫПОЛНЕНИЕ ЗАДАЧИ ЗАВЕРШЕНО-----\n")
class ConcatCsv(QThread):
    def __init__(self, parent=None):
        super(ConcatCsv, self).__init__(parent)
        self.value = 0
        self.input_folder = ""      
        self.output_folder = ""     
        self.output_name = ""       
    def run(self):
        print("-----КОНКАТЕНАЦИЯ ФАЙЛОВ .csv-----\n")
        try:
            concat_csv(self.input_folder, self.output_folder, self.output_name)
        except Exception as ex:
            print("Возникло непредвиденное исключение. Перепроверьте исходные данные.\n", ex)
        print("\n-----ВЫПОЛНЕНИЕ ЗАДАЧИ ЗАВЕРШЕНО-----\n")
...
def merge_csv(csv_path_1, csv_path_2, csv_output_path, csv_output_name,
    ...
    except Exception as ex: print("Не получилось объединить файлы\n", ex)
def concat_csv(input_folder, output_folder, file_name):
    folder_name = input_folder + '/'
    results = []
    if not os.path.exists(output_folder):
        create_dir(output_folder)
        dst_path = path.join(output_folder, file_name)
    else:
        dst_path = path.join(output_folder, file_name)
        if path.isfile(dst_path):
            cnt_of_files_in_dir = len(listdir(output_folder))
            new_name = str(int(cnt_of_files_in_dir / 3) + 1) + '_' + file_name
            dst_path = path.join(output_folder, new_name)
    for f in os.listdir(folder_name):
        if f.endswith('.csv'):
            results.append(folder_name + str(f))
    if len(results) != 0:
        try:
            combined_csv = pd.concat([pd.read_csv(f) for f in results])
            combined_csv.to_csv(dst_path, index=False)
            print("Успешно!\n")
            for i in range (0, len(results)):
                print(results[i])
        except Exception as ex:
            print(f'Объединение файлов не выполнено..\n\n{ex}')
    else:
        print("В указанной папке не найдено ни одного файла [.csv]")