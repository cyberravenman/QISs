...
echo '   \$$$$$$\ \$$$$$$  \$$$$$$  \$$$$$$$   '    
echo '       \$$$                              '
echo '                         launcher v.0.3.9'
echo 'Che_Guevara_22 -_-     CyberRavenMan  *_*'

if [[ $1 == "del_venv" ]]; then
	rm -rf $PWD/QISs_v.0.3.9-venv
...
if [ -d "$PWD/QISs_v.0.3.9-venv" ]; then
	echo '[*]: Activate Python virtual env..'
	source $PWD/QISs_v.0.3.9-venv/bin/activate &>-
	echo '[+]: Done'
else
	echo '[*]: Updating..'
	sudo apt update
	...
	python3 -m venv QISs_v.0.3.9-venv
	echo '[+]: Done'
...
python3 QIS_Benchmark_v.0.3.9.py
